...
public class TypeCheck extends Tree.Visitor {
	...
	public void visitTernary(Tree.Ternary expr) {
		checkTestExpr(expr.first); 
		expr.second.accept(this); 
		expr.third.accept(this);
		if (expr.first.type.equal(BaseType.BOOL) &&
				expr.second.type.equal(expr.third.type)) {
			expr.type = expr.third.type;
		}
		else {
			expr.type = BaseType.ERROR;
		}
		if (!expr.second.type.equal(expr.third.type) && 
				!expr.second.type.equal(BaseType.ERROR) && 
				!expr.third.type.equal(BaseType.ERROR)) {
			issueError(new IncompatTerOpError(expr.getLocation(), expr.second.type.toString(), expr.third.type.toString()));
		}
	}
	...
	public void visitSwitch(Tree.Switch switchStmt) {
		...
		if (!switchStmt.expr.type.equal(BaseType.INT))
			issueError(new BadSwitchVarError(switchStmt.expr.loc));
		for (Tree case_ : switchStmt.cases)
			case_.accept(this);
		breaks.pop();
	}
	
	public void visitCase(Tree.Case caseStmt) {	
		caseStmt.con.accept(this);
		if (!caseStmt.con.type.equal(BaseType.INT))
			issueError(new BadCaseLabelError(caseStmt.con.loc));
		for (Tree block_ : caseStmt.blocks)
			block_.accept(this);
	}
	
	public void visitDefault(Tree.Default defaultStmt) {
		for (Tree block_ : defaultStmt.blocks)
			block_.accept(this);
	}
	...
	private Type checkBinaryOp(Tree.Expr left, Tree.Expr right, int op, Location location) {
		...
		case Tree.BF:
			...
			Class first = ((ClassType) left.type).getSymbol();
			Class second = ((ClassType) right.type).getSymbol();
			while (first.getOrder() < second.getOrder()) {
