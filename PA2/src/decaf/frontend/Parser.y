...
Expr            :	LValue
				...
				|	Expr '?' Expr ':' Expr
                	{
						$$.expr = new Tree.Ternary(Tree.CHOOSE, $1.expr, $3.expr, $5.expr, $1.loc);
					}
				|	Expr BF Expr
					{
						$$.expr = new Tree.Binary(Tree.BF, $1.expr, $3.expr, $2.loc);
					}
                ;
...
SwitchStmt		:   SWITCH '(' Expr ')' '{' CaseDefault '}' 
					{
						$$.stmt = new Tree.Switch($3.expr, $6.cases, $1.loc);	
					}
				;
				
CaseStmt		:   CASE Constant ':' StmtList
					{
						$$.cs = new Tree.Case($2.expr, $4.slist, $1.loc);
					}
				;
...
CaseStmtList	:	CaseStmtList CaseStmt
                ...
                |   /* empty */             %prec EMPTY
                    {
                        $$ = new SemValue();
                        $$.cases = new ArrayList<Tree>();
                    }
				;

DefaultStmt		:	DEFAULT ':' StmtList
					{
						$$.cs = new Tree.Default($3.slist, $1.loc);
					}
				|   /* empty */				%prec EMRTY
					{
						$$ = new SemValue();
